...
export type SeatSwapRegionMode = 'browse' | 'filter' | 'published' | 'select-current' | 'select-desired' | 'review'

export interface SeatSwapRegionVisualState {
  highlightCurrent: boolean
  highlightDesired: boolean
  highlightFilter: boolean
  reviewMuted: boolean
  reviewCurrent: boolean
  reviewDesired: boolean
  dimmed: boolean
  disabled: boolean
}

export function resolveSeatSwapRegionVisualState(input: {
...
}): SeatSwapRegionVisualState {
  const mode = input.mode
  const isCurrent = !!input.currentKey && input.currentKey === input.key
  const isDesired = (input.desiredKeys || []).includes(input.key)
  const isStagedCurrent = !!input.stagedCurrentKey && input.stagedCurrentKey === input.key
  const isStagedDesired = (input.stagedDesiredKeys || []).includes(input.key)
  const isFilterMatched = !!input.filterKey && input.filterKey === input.key
  ...
  const highlightDesired = (isPublished && isDesired) || (isSelectDesired && isStagedDesired)
  const highlightFilter = isFilter && isFilterMatched

  const dimmed = shouldDimSeatSwapRegion({
    mode,
    hasFilterKey: Boolean(input.filterKey),
    isFilterMatched,
    hasStagedCurrentKey: Boolean(input.stagedCurrentKey),
    isStagedCurrent,
    hasStagedDesiredKeys: (input.stagedDesiredKeys || []).length > 0,
    isStagedDesired,
    isCurrent,
    isDesired,
  })
  ...
  return {
    ...
    highlightDesired,
    highlightFilter,
    reviewMuted: isReview && !isCurrent && !isDesired,
    reviewCurrent: isReview && isCurrent,
    reviewDesired: isReview && isDesired,
    dimmed,
    disabled: isSelectDesired && isStagedCurrent,
  }
}

export interface SeatSwapRegionChipGroup {
  key: string
  label: string
  items: TicketWatchRegion[]
}

const chipGroupMeta: Array<{ key: string; label: string; ring: string; side: string }> = [
  { key: 'inner-top', label: '下层 · 北', ring: 'inner', side: 'top' },
  { key: 'inner-right', label: '下层 · 东', ring: 'inner', side: 'right' },
  { key: 'inner-bottom', label: '下层 · 南', ring: 'inner', side: 'bottom' },
  { key: 'inner-left', label: '下层 · 西', ring: 'inner', side: 'left' },
  { key: 'outer-top', label: '上层 · 北', ring: 'outer', side: 'top' },
  { key: 'outer-right', label: '上层 · 东', ring: 'outer', side: 'right' },
  { key: 'outer-bottom', label: '上层 · 南', ring: 'outer', side: 'bottom' },
  { key: 'outer-left', label: '上层 · 西', ring: 'outer', side: 'left' },
  { key: 'vip', label: 'VIP', ring: 'vip', side: 'vip' },
]

export function groupSeatSwapRegionsForChips(regions: TicketWatchRegion[]): SeatSwapRegionChipGroup[] {
  const buckets = new Map<string, TicketWatchRegion[]>()

  for (const region of regions) {
    const name = region.block_name
    const layout = resolveSeatSwapRegionLayout(name) || resolveSeatSwapRegionLayout(region.block_key || '')
    if (!layout) continue
    const bucketKey = layout.ring === 'vip' ? 'vip' : `${layout.ring}-${layout.side}`
    const bucket = buckets.get(bucketKey)
    if (bucket) {
      bucket.push(region)
    } else {
      buckets.set(bucketKey, [region])
    }
  }

  return chipGroupMeta
    .filter((meta) => buckets.has(meta.key))
    .map((meta) => ({
      key: meta.key,
      label: meta.label,
      items: [...(buckets.get(meta.key) || [])].sort((a, b) =>
        a.block_name.localeCompare(b.block_name, 'zh-Hans-CN', { numeric: true }),
      ),
    }))
}
