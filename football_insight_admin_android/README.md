...
内网 Debug 包默认连接 local233 管理后端：

```text
http://172.16.60.233:18092/
```

该后端通过私有 SSH 隧道直接连接生产 `football_data`。在 App 中调整会员、禁用或恢复用户会立即修改生产数据。

Release 构建默认生产地址（仅用于未来生产发布）：

```text
https://match.oryjk.cn/
```

不要在真机上使用 `localhost`；它指向手机自身。Android 模拟器访问开发机测试服务时使用：
